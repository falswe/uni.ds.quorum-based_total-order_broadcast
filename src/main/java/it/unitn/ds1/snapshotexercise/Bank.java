...
public class Bank extends AbstractActor {
  ...
  private void onMoney(Money msg) {
    ...
    //  v-- Are we performing a snapshot?
    //                              v-- Did we already receive the token for this link?
  ...
  private void onToken(Token token) {
    ...
    if (!this.stateBeingCaptured) {
      ...
      captureState();
      sendTokens();
    } else if (this.tokensReceived.containsAll(this.peers)) {
      System.out.println("Bank " + id +
              " snapId: "+ snapId +
              " state: " + (this.capturedBalance + this.moneyInTransit));
